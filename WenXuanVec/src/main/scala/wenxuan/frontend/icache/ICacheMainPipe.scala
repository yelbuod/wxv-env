...
import wxv_util.{ResultHoldBypass, ValidHoldBypass}
import utility._
...
{
  val req  = Flipped(DecoupledIO(new FtqToICacheRequestBundle))
...
{
  val toMSHR = DecoupledIO(new ICacheMissReq)
  val fromMSHR = Flipped(DecoupledIO(new ICacheMissResp))
}
...
class ICacheMainPipeInterface(implicit p: Parameters) extends ICacheBundle {
  ...
  val IPQRead       = Flipped(new IPQRead)
  val IPFReplacer         = Flipped(new IPFReplacer)
  // Filter the same req in the prefetch as in the mainpipe
...
class ICacheMainPipe(implicit p: Parameters) extends ICacheModule {
  ...
  val (toIPQ, fromIPQ) = (io.IPQRead.req, io.IPQRead.resp)
...
//    1, 300, 1, right_strict = true)
  ...
  // Received no miss signal from ITLB at the beginning of s1 stage or during the s1 waiting stage
  ...
  val s1_ipf_hit_latch = VecInit((0 until PortNumber).map(i => ValidHoldBypass(infire = s1_ipf_hit(i), outfire = s1_fire, flush = false.B)))
  val s1_ipf_data = VecInit((0 until PortNumber).map(i => ResultHoldBypass(data = fromIPF(i).cacheline, valid = s1_ipf_hit(i))))

  /** check in IPQ, if hit, wait until prefetch port hit */
  (0 until PortNumber).foreach { i =>
    toIPQ(i).valid := tlb_valid_tmp(i)
    toIPQ(i).bits.paddr := s1_req_paddr(i)
  }
  val s1_ipq_hit = VecInit((0 until PortNumber).map(i => toIPF(i).valid && fromIPQ(i).ipq_hit))
  val s1_ipq_hit_latch = VecInit((0 until PortNumber).map(i => ValidHoldBypass(infire = s1_ipq_hit(i), outfire = s1_fire, flush = false.B)))
  val wait_ipq = VecInit((0 until PortNumber).map(i => toIPF(i).valid && fromIPQ(i).ipq_hit && !fromIPQ(i).data_valid))
  val wait_ipq_latch = VecInit((0 until PortNumber).map(i => ValidHoldBypass(infire = wait_ipq(i), outfire = s1_fire || fromIPQ(i).data_valid, flush = false.B)))
  val s1_ipq_data = VecInit((0 until PortNumber).map(i => ResultHoldBypass(data = fromIPQ(i).cacheline, valid = (s1_ipq_hit(i) || wait_ipq_latch(i)) && fromIPQ(i).data_valid)))

  val s1_wait = (0 until PortNumber).map(i => wait_ipq_latch(i) && !fromIPQ(i).data_valid).reduce(_ || _)

  val s1_prefetch_hit = VecInit((0 until PortNumber).map(i => s1_ipf_hit_latch(i) || s1_ipq_hit_latch(i)))
  val s1_prefetch_hit_data = VecInit((0 until PortNumber).map(i => Mux(s1_ipf_hit_latch(i), s1_ipf_data(i), s1_ipq_data(i))))
  ...
  val isWriteICacheTable = WireInit(Constantin.createRecord("isWriteICacheTable" + p(WXVTileKey).hartId.toString))
  val ICacheTable = ChiselDB.createTable("ICacheTable" + p(WXVTileKey).hartId.toString, new ICacheDB)

  val ICacheDumpData_req0 = Wire(new ICacheDB)
  ICacheDumpData_req0.blk_paddr := get_block(s1_req_paddr(0))
  ICacheDumpData_req0.blk_vaddr := get_block(s1_req_vaddr(0))
  ...
  ICacheDumpData_req1.blk_paddr := get_block(s1_req_paddr(1))
  ICacheDumpData_req1.blk_vaddr := get_block(s1_req_vaddr(1))
...
//  }
//
//  XSPerfAccumulate("mainPipe_stage1_block_by_ipq_cycles", s1_valid && s1_wait)
  ...
  val s2_flush_latch = ValidHoldBypass(infire=io.flush && s2_valid, outfire=s2_fire, flush=false.B)
  ...
  class MissSlot(implicit p: Parameters) extends ICacheBundle {
    ...
    val dataline = RegInit(0.U(blockBits.W))
    val corrupt = RegInit(false.B)
    val finish = RegInit(true.B)
    val valid = RegInit(false.B)

    def data_vec = dataline.asTypeOf(Vec(2, UInt((blockBits / 2).W)))
  ...
  (0 until PortNumber).map { i =>
    when((missStateQueue(i) === m_wait_resp) && fromMSHR(i).fire) {
      ...
      missSlot(i).dataline := fromMSHR(i).bits.dataline
  ...
  (0 until PortNumber).map { i =>
    ...
    else        toIFU(i).valid := s2_fire && s2_double_line && !s2_flush_latch
