...
//  with MemoryOpConstants
{
  ...
  io.ongoing_req.paddr :=  addrAlignByte(addr=req.paddr, bytes=blockBytes)
    ...
    is(s_wait_mem_grant) {
      when(edge.hasData(io.mem_grant.bits)) {
        when(io.mem_grant.fire) {
          ...
          when(readBeatCnt === (refillCycles - 1).U) { // refill complete
  ...
  val getBlock = edge.Get(
    fromSource = io.id,
    toAddress = addrAlignByte(addr=req.paddr, bytes=blockBytes),
  ...
  io.resp.bits.dataline := respDataReg.asUInt
  io.resp.bits.corrupt := req_corrupt

  // no write refill dataline to meta & data if fencei flush
