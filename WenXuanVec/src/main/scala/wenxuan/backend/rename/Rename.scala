...
import utility._
import utils._
import wenxuan.backend.{InstCtrlPacket, MicroOp, SnapshotPort, RobCommitIO}
import wenxuan.common._
import wenxuan.backend.decode.FusionDecodeInfo
import wenxuan.commonType.{FuType, SelImm, SrcType}
import wenxuan.functionOpType.LSUOpType
import wenxuan.backend.decode.{FusionDecodeInfo, Imm_I, Imm_LUI_LOAD, Imm_U}
import wenxuan.backend.rob.RobPtr

class Rename(implicit p: Parameters) extends WXModule with HasCircularQueuePtrHelper with HasPerfEvents {
  val io = IO(new Bundle() {
    val redirect = Flipped(ValidIO(new Redirect))
    val robCommits = Input(new RobCommitIO)
    // from decode
    val in = Vec(RenameWidth, Flipped(DecoupledIO(new InstCtrlPacket)))
    val fusionInfo = Vec(DecodeWidth - 1, Flipped(new FusionDecodeInfo))
    // ssit read result
    val ssit = Vec(RenameWidth, Input(new SSITEntry))
    // waittable read result
    val waittable = Vec(RenameWidth, Input(Bool()))
    // from rename table
  ...
  // decide if given instruction needs allocating a new physical register
  // (InstCtrlPacket: from decode; RobCommitInfo: from rob)
  def needDestReg[T <: InstCtrlPacket](fp: Boolean, x: T): Bool = {
  ...
  /**
   *  speculatively assign the instruction with an robIdx by robIdx Counter
   */
...
//  require(RenameWidth >= CommitWidth)
  ...
  // write intRat / fpRat speculative
  val intRatSpecWen = Wire(Vec(RenameWidth, Bool()))
  val fpRatSpecWen = Wire(Vec(RenameWidth, Bool()))

  // uop calculation
  for (i <- 0 until RenameWidth) {
    // no valid instruction from decode stage || all resources (dispatch1 + both free lists) ready
    io.in(i).ready := !hasValid || canOut // inform decode stage ready

    /**
     * Micro Operation Generate
     */
    uops(i).instPacket := io.in(i).bits.instPacket
    uops(i).ctrl := io.in(i).bits.ctrl

    // update instPacket according to ssit result
    uops(i).instPacket.storeSetHit := io.ssit(i).valid
    uops(i).instPacket.loadWaitStrict := io.ssit(i).strict && io.ssit(i).valid
    uops(i).instPacket.ssid := io.ssit(i).ssid
    // update instPacket according to waittable result
    uops(i).instPacket.loadWaitBit := io.waittable(i)
    ...
    intRatSpecWen(i) := needIntDest(i) && intFreeList.io.canAllocate && intFreeList.io.doAllocate && !io.robCommits.isWalk && !io.redirect.valid
    fpRatSpecWen(i) := needFpDest(i) && fpFreeList.io.canAllocate && fpFreeList.io.doAllocate && !io.robCommits.isWalk && !io.redirect.valid
    /*        RAT Update : write new mapping     */
    io.intRenamePorts(i).wen := intRatSpecWen(i)
    io.intRenamePorts(i).addr := uops(i).ctrl.ldest
    io.intRenamePorts(i).data := intFreeList.io.allocatePhyReg(i) // io.out(i).bits.pdest

    io.fpRenamePorts(i).wen := fpRatSpecWen(i)
  ...
  /**
   * Solve Dependencies inside instPacket
   * How to set psrc:
  ...
  /**
   * Snapshot signal generate here for all arch resources : robIdx, freelist Idx, renameTable and robPtr
   *  Generate Condition: snapshotCtr == 0 for a certain interval of generation and hasCFI insure generate at CFI instr
   */
  val hasCFI = VecInit(io.in.map(in => (!in.bits.instPacket.pd.notCFI || FuType.isJumpExu(in.bits.ctrl.fuType)) && in.fire)).asUInt.orR
  ...
  /**
   * ROB WALK resume: WALK to resume freelist allocation after redirect
   */
  ...
  for (i <- 0 until CommitWidth) {
    ...
    /*      II. Free List Update : free old alloc phy regs      */
    fpFreeList.io.freeReq(i)  := RegNext(commitValid && needDestRegCommit(fp=true, io.robCommits.info(i)), false.B)
   ...
   */
  def printRenameInfo(in: DecoupledIO[InstCtrlPacket], out: DecoupledIO[MicroOp]) = {
    XSInfo(out.fire, p"pc:${Hexadecimal(in.bits.instPacket.pc)} in(${in.valid},${in.ready}) " +
