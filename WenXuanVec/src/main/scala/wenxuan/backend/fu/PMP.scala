...
import utility.{MaskedRegMap, OneHot, ParallelPriorityMux}
...
abstract class PMPWXModule(implicit p: Parameters) extends WXModule with PMPConst
...
class PMP(implicit p: Parameters) extends PMPWXModule with PMPMethod with PMAMethod with HasCSRConst {
...
class PMPReqBundle(lgMaxSize: Int = 3)(implicit p: Parameters) extends PMPBundle {
  val paddr = Output(UInt(PMPAddrBits.W))
  ...
  with PMACheckMethod
{
  ...
  val res_pmp = pmp_match_res(leaveHitMux, io.req.valid)(req.paddr, req.size, io.check_env.pmp, io.check_env.mode, lgMaxSize)
  val res_pma = pma_match_res(leaveHitMux, io.req.valid)(req.paddr, req.size, io.check_env.pma, io.check_env.mode, lgMaxSize)
