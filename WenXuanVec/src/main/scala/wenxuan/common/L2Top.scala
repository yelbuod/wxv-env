...
import wenxuan.cache.L1BusErrorUnitInfo
import coupledL2.{L2ParamKey, CoupledL2}
import system.HasSoCParameter
import utility.{DelayN, ResetGen, TLClientsMerger, TLEdgeBuffer, TLLogger}
import top.BusPerfMonitor

//class L2BusErrorUnitInfo(implicit val p: Parameters) extends Bundle with HasSoCParameter {
//  val ecc_error = Valid(UInt(soc.PAddrBits.W))
//}

class WXL1BusErrors()(implicit val p: Parameters) extends BusErrors {
  val icache = new L1BusErrorUnitInfo
  val dcache = new L1BusErrorUnitInfo
//  val l2 = new L2BusErrorUnitInfo
  ...
  with HasSoCParameter
{
  ...
  val beu = LazyModule(new BusErrorUnit(
    new WXL1BusErrors(), BusErrorUnitParams(0x38010000)
  ))
  ...
  val l1d_logger = TLLogger(s"L2_L1D_${HartId}", enbale_tllog)
  val l1i_logger = TLLogger(s"L2_L1I_${HartId}", enbale_tllog)
  val ptw_logger = TLLogger(s"L2_PTW_${HartId}", enbale_tllog)
  ...
  val l2cache = tileParams.l2cacheOpt.map(l2param =>
  ...
  val l2_binder = tileParams.l2cacheOpt.map(param => BankBinder(param.banks, 64))
...
class L2TopImp(outer: L2Top) extends LazyModuleImp(outer)
  with HasTileParameters
  with HasSoCParameter
{
  ...
  if (outer.l2cache.isDefined) {
    l2_hint := outer.l2cache.get.module.io.l2_hint
    // debugTopDown <> l2cache.get.module.io.debugTopDown
    outer.l2cache.get.module.io.debugTopDown.robHeadPaddr := DontCare
    outer.l2cache.get.module.io.debugTopDown.robHeadPaddr.head := debugTopDown.robHeadPaddr
    debugTopDown.l2MissMatch := outer.l2cache.get.module.io.debugTopDown.l2MissMatch.head
  } else {
