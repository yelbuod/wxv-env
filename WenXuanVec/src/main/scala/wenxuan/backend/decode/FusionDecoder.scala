...
import utils.{XSError, XSPerfAccumulate}
import wenxuan.commonType.{FuType, SrcType}
import wenxuan.functionOpType.{ALUOpType, FuOpType, MDUOpType}
import wenxuan.backend.CtrlSignals
import wenxuan.common.WXBundle

abstract class BaseFusionCase(pair: Seq[Valid[UInt]])(implicit p: Parameters)
  extends RVDecodeConstants {
...
class FusionDecoder(implicit p: Parameters) extends WXBundle {
