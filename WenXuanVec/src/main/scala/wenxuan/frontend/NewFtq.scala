...
class Ftq(implicit p: Parameters) extends WXModule with HasBPUConst
{
  val io = IO(new Bundle {
  ...
  // **************************** FTQ <> BPU ****************************
  // **********************************************************************
  val allowBpuIn = WireInit(false.B)
  ...
  val enq_fire = io.fromBpu.resp.fire && allowBpuIn // from bpu s1/s2/s3
